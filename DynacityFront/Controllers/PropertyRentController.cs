...
namespace DynacityFront.Controllers
{
    public class PropertyRentController : Controller
    {
        ...
        public PropertyRentController(IRentService rentService, IMapper mapper, IWebHostEnvironment webHost)
        {
        ...
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? page)
        {
            ...
            var response = await _rentService.GetAllAsync<APIResponse>(HttpContext.Session.GetString(SD.SessionToken));
        ...
        {
            PropertyRentCreateDTO bothVM = new PropertyRentCreateDTO();
        ...
        public async Task<IActionResult> Create(PropertyRentCreateDTO model)
        {
            if (ModelState.IsValid)
            {
                ...
                var Master2 = new RentCreateDTO()
                {

                    StartDate = model.Rent.StartDate,
                    EndDate = model.Rent.EndDate,
                    RentPrice = model.Rent.RentPrice,
                    IsActive = model.Rent.IsActive,
                    RentDiscription = model.Rent.RentDiscription,
                ...
                var Master3 = new PropertyRentCreateDTO()
                {
                    Property = Master,
                    Rent = Master2
                ...
                var response = await _rentService.CreateAsync<APIResponse>(Master3, HttpContext.Session.GetString(SD.SessionToken));
                if (response != null && response.IsSuccess)
                {
        ...
        {
            var response = await _rentService.GetAsync<APIResponse>(RentId, HttpContext.Session.GetString(SD.SessionToken));
            if (response != null && response.IsSuccess)
            {
                RentShowDTO model = JsonConvert.DeserializeObject<RentShowDTO>(Convert.ToString(response.Result));
                return View(model);
            }
            return NotFound();
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Update(RentUpdateDTO model)
        {
            if (ModelState.IsValid)
            {
                TempData["success"] = "Rent Updated successfully!";
                var response = await _rentService.UpdateAsync<APIResponse>(model, HttpContext.Session.GetString(SD.SessionToken));
                if (response != null && response.IsSuccess)
                {
                    return RedirectToAction(nameof(Index));
                }
        ...
        public async Task<IActionResult> Delete(RentDTO model)
        {
            ...
            var response = await _rentService.DeleteAsync<APIResponse>(model.RentId, HttpContext.Session.GetString(SD.SessionToken));
            if (response != null && response.IsSuccess)
            {
        ...
        public async Task<IActionResult> Details(int RentId)
        {
            RentShowDTO dto = new();
            var response = await _rentService.GetAsync<APIResponse>(RentId, HttpContext.Session.GetString(SD.SessionToken));
            if (response != null && response.IsSuccess)
            {
                 dto = JsonConvert.DeserializeObject<RentShowDTO>(Convert.ToString(response.Result));
                return View(dto);
            }
            else
            {
